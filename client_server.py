...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.server_label = tk.Label(self.login_frame, text="Адрес сервера:")
        self.server_label.pack(side="top")

        self.server_entry = tk.Entry(self.login_frame)
        self.server_entry.pack(side="top")
    ...
    def login(self):
        ...
        response = requests.post(f'http://{server}:5000/login', json={'username': username, 'password': password})
    ...
    def register(self):
        ...
        password = self.password_entry.get()
        server = self.server_entry.get()
        response = requests.post(f'http://{server}:5000/register', json={'username': username, 'password': password})
        if response.json()['success']:
    ...
    def get_data(self):
        server = self.server_entry.get()
        response = requests.get(f'http://{server}:5000/get_data')
